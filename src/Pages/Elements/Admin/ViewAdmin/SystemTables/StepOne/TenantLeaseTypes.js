...
class ViewLeaseTypes extends Component {
    constructor(props){
		super(props);
		const ServerPath = JSON.parse(localStorage.getItem('ServerPath'));
		this.state = {
			ServerPath: ServerPath,
            formData:{},LeaseType:[],EditLeaseType:[],editId:'',AddForm:true,Editform:false,Table:false
		}
     }
    ...
    EditId = id =>{
        ...
        fetch('http://'+this.state.ServerPath+'/api/editSystemTableList/'+id,{
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({EditLeaseType : data.body})
				}
            })
    }
    componentDidMount(){
		this._isMounted = true;
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({LeaseType : data.body})
    ...
    render() {
        const {LeaseType,EditLeaseType} = this.state;
        return (
            <Fragment>
                        ...
                        <Col className="pad0">
							<Card className="main-card mb-3">
								<CardBody className="pad0 min-vh-100">
                                        <div className="card-header"><div className="ml-3">Tenant Lease Types</div> </div>
                                        {this.state.Table?
												...
												<table className="align-middle mb-0 table table-borderless table-striped table-hover table_clients">
													<thead>
														<tr className="heading_color">
															...
															<th className="text-center">Lease Type</th>
															<th className="text-center">Action</th>
														</tr>
													</thead>
													<tbody>
														{LeaseType.map((fields,index) =>
														<tr key={index}>
															<td className="text-center">{index+1}</td>
															<td className="text-center">{fields.LeaseType}</td>
                                           ...
                                           <Form className="col-md-12" onSubmit={this.handleSubmit}>
                                            ...
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-6">
                                                    <Label className="pad0"> Lease Type :</Label>
                                                    <Input type="text" onChange={this.handleInputChange} name="LeaseType"/>
                                                </Label>
                                        ...
                                        <div>
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-6">
                                                    <Label className="pad0"> Lease Type :</Label>
                                                    <Input type="text" defaultValue={EditLeaseType.LeaseType} onChange={this.handleInputChange} name="LeaseType"/>
                                                </Label>
                                                <Label className="col-md-2">
                                                <Label className="pad0"></Label>
