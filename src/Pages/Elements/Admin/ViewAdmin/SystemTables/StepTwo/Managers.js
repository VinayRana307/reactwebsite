...
class ViewManagers extends Component {
    constructor(props){
		...
		this.state = {
            ...
            formData:{},ManagerList:[],EditManager:[],editId:'',AddForm:true,Editform:false,Table:false,
     }
    }
    ...
    EditId = id =>{
        ...
        fetch('http://'+this.state.ServerPath+'/api/editSystemTableList/'+id,{
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({EditManager: data.body})
				}
            })
    }
    componentDidMount(){
		this._isMounted = true;
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({ManagerList : data.body})
    ...
    render() {
        const {ManagerList,EditManager} = this.state;
        return (
            <Fragment>
                        ...
                        <Col className="pad0">
							<Card className="main-card mb-3">
								<CardBody className="pad0 min-vh-100"> 
                                        <div className="card-header pad0">Manager
                                        ...
                                        <Label className="pad0 ml-3"><b>This code is used within the Repair Order facility on the property/supplier records.</b></Label>
                                        {this.state.Table?
                                        <div className="table-responsive">
												<table className="align-middle mb-0 table table-borderless table-striped table-hover table_clients">
													...
													<tbody>
														{ManagerList.map((fields,index) =>
														<tr key={index}>
															<td className="text-center">{index+1}</td>
															<td className="text-center">{fields.Manager}</td>
                                           ...
                                           <Form className="col-md-12" onSubmit={this.handleSubmit}>
                                            ...
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-5">
                                                    <Label className="pad0"> Manager :</Label>
                                                    <Input type="text" onChange={this.handleInputChange} name="Manager"/>
                                                </Label>
                                                <Label className="col-md-5">
                                                    <Label className="pad0"> Management Type :</Label>
                                                    <Input type="text" onChange={this.handleInputChange} name="ManagementType"/>
                                                </Label>
                                        ...
                                        <div>
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-5">
                                                    <Label className="pad0"> Manager :</Label>
                                                    <Input type="text" defaultValue={EditManager.Manager} onChange={this.handleInputChange} name="Manager"/>
                                                </Label>
                                                <Label className="col-md-5">
                                                    <Label className="pad0"> Management Type :</Label>
                                                    <Input type="text" onChange={this.handleInputChange} name="ManagementType"/>
