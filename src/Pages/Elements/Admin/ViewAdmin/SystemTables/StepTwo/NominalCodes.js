...
class ViewNominalCodes extends Component {
    constructor(props){
		super(props);
		const ServerPath = JSON.parse(localStorage.getItem('ServerPath'));
		this.state = {
			ServerPath: ServerPath,
            formData:{},NominalCodeList:[],EditNominalCode:[],editId:'',AddForm:true,Editform:false,Table:false
		}
     }
    ...
    EditId = id =>{
        ...
        fetch('http://'+this.state.ServerPath+'/api/editSystemTableList/'+id,{
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({EditNominalCode: data.body})
				}
            })
    }
    componentDidMount(){
		this._isMounted = true;
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({NominalCodeList : data.body})
                    if(this.state.NominalCodeList == ''){
    ...
    render() {
        const {NominalCodeList,EditNominalCode} = this.state;
        return (
            <Fragment>
                        ...
                        <Col className="pad0">
							<Card className="main-card mb-3">
								<CardBody className="pad0 min-vh-100">
                                        <div className="card-header"><div className="ml-3">Nominal Codes</div> </div>
                                        <Label className="pad0 ml-3"><b>This is where you can enter the nominal analysis codes that the system uses to allocate income, expenditure, tenant fees and property/client fees. Nominal codes will be discussed at the initial training session.</b></Label>
                                        {this.state.Table?
                                        <div className="table-responsive">
												<table className="align-middle mb-0 table table-borderless table-striped table-hover table_clients">
													...
													<tbody>
														{NominalCodeList.map((fields,index) =>
														<tr key={index}>
															<td className="text-center">{index+1}</td>
															<td className="text-center">{fields.NominalCode}</td>
                                           ...
                                           <Form className="col-md-12" onSubmit={this.handleSubmit}>
                                            ...
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-6">
                                                    <Label className="pad0"> Nominal Code :</Label>
                                                    <Input type="text" onChange={this.handleInputChange} name="NominalCode"/>
                                                </Label>
                                        ...
                                        <div>
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-6">
                                                    <Label className="pad0"> Nominal Code :</Label>
                                                    <Input type="text" defaultValue={EditNominalCode.NominalCode} onChange={this.handleInputChange} name="NominalCode"/>
                                                </Label>
                                                <Label className="col-md-2">
                                                <Label className="pad0"></Label>
...
export default ViewNominalCodes;