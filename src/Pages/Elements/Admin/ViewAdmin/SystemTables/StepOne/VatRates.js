...
class ViewVatRates extends Component {
    constructor(props){
		super(props);
		const ServerPath = JSON.parse(localStorage.getItem('ServerPath'));
		this.state = {
			ServerPath: ServerPath,
            formData:{},VatRateList:[],EditVatRate:[],editId:'',AddForm:true,Editform:false,Table:false,Addbtn:true
		}
     }
    ...
    EditId = id =>{
        ...
        fetch('http://'+this.state.ServerPath+'/api/editSystemTableList/'+id,{
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({EditVatRate : data.body})
				}
            })
    }
    componentDidMount(){
		this._isMounted = true;
			...
			.then((data) => {
			if (this._isMounted) {
                this.setState({VatRateList : data.body})
                if(this.state.VatRateList == ''){
    ...
    render() {
        const {VatRateList,EditVatRate} = this.state;
        return (
            <Fragment>
                        ...
                        <Col className="pad0">
							<Card className="main-card mb-3">
								<CardBody className="pad0 min-vh-100">
                                        <div className="card-header"><div className="ml-3">VAT Rates </div> </div>
                                        <Label className="pad0 ml-3"><b>This is where you can set up and change the vat rates.</b></Label>
                                        {this.state.Table?
                                        <div className="table-responsive">
												<table className="align-middle mb-0 table table-borderless table-striped table-hover table_clients">
													...
													<tbody>
														{VatRateList.map((fields,index) =>
														<tr key={index}>
															<td className="text-center">{index+1}</td>
															<td className="text-center">{fields.VatAmount}</td>
                                           ...
                                           <Form className="col-md-12" onSubmit={this.handleSubmit}>
                                            ...
                                            <FormGroup className="col-md-12 mt-4"> 
                                                <Label className="col-md-6">
                                                    <Label className="pad0"> Vat Amount :</Label>
                                                    <Input type="text" onChange={this.handleInputChange} name="VatAmount"/>
                                                </Label>
                                        ...
                                        <div>
                                            <FormGroup className="col-md-12 mt-3"> 
                                                <Label className="col-md-6">
                                                    <Label className="pad0"> Vat Amount :</Label>
                                                    <Input type="text" defaultValue={EditVatRate.VatAmount} onChange={this.handleInputChange} name="VatAmount"/>
                                                </Label>
                                                <Label className="col-md-2">
                                                <Label className="pad0"></Label>
